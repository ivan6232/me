...
let elemurl = $.getdata('elemurl')
let elemhd = $.getdata('elemhd')
//let elemkey = '', uid = ''



// simple sign in mission
...
// page view mission
//const url10 = 'https://h5.ele.me/restapi/biz.svip_scene/svip/engine/xSupply?params[]=%7B%22tagCode%22:%22224166%22,%22extra%22:%7B%22missionDefId%22:1036001,%22missionCollectionId%22:36,%22missionType%22:%22PAGEVIEW%22%7D%7D';
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        for (let i = 2; i <= elemcount; i++) {
            elemurlArr.push($.getdata(`elemurl${i}`))
        ...
        for (let i = 0; i < elemhdArr.length; i++) {
            if (elemhdArr[i]) {
                ...
                await elem_mission_1()
                await elem_mission_2()
...
// check balance
function elembalance(timeout = 0) {
    return new Promise((resolve) => {
        ...
        $.get(url, async (err, resp, data) => {
            try {
                ...
                $.msg('吃货豆数量：' + data.accountInfos[0].count)
            } catch (e) {
                //$.logErr(e, resp);
            } finally {
                resolve()
...
function elem_mission_1(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url: 'https://h5.ele.me/restapi/biz.svip_scene/svip/engine/xSupply?params[]=%7B%22tagCode%22:%22223166%22,%22extra%22:%7B%22missionDefId%22:1390001,%22missionCollectionId%22:36,%22missionType%22:%22SIMPLESIGNIN%22%7D%7D',
            headers: {
                headers: JSON.parse(elemhd),
            },
        }
        $.get(url, async (err, resp, data) => {
            try {
                ...
                if (data.data[0].attribute.msgCode == "4000") {
                    console.log('任务-' + missionDefId + ':' + data.data[0].attribute.message + `\n`)
                } else {
                    console.log('任务-' + missionDefId + data.data[0].attribute.message + '：获得' + data.data[0].attribute.value + '吃货豆' + `\n`)
                }
            } catch (e) {
                //$.logErr(e, resp);
            } finally {
